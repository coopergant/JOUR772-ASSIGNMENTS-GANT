...
It now is 54000! Showing us all of the options that have at LEAST 15 seats OR is a theatre class.
...
***When it comes to filters, OR is additive; AND is restrictive. #cooper_remember_plz #added_2_notes***
...
**Answer** It's all listed as the first of the month lol. I doubt that's true. Prob a data error.
...
**Answer**\
It sums the data of the votes then divides and calculates by proportion and not just overall.
...
\*\*Answer\*\* It now shows the counties where Gov. Moore got the highest percentage of votes at the top
...
**Answer**\
More interesting in my opinion because it highlights the counties where Moore struggled to get real traction since it's now proportional.
...
I used this [site here](https://stringr.tidyverse.org/reference/case.html) and found `str_to_upper()` converts to upper case, `str_to_lower()` converts to lower case, `str_to_title()` converts to title case (where only the first letter of each word is capitalized), and `str_to_sentence()` convert to sentence case (where only the first letter of sentence is capitalized).
...
**Answer\
It is creating a new column for the state listed, looking for each state and adding the state's abrieviation in the column that corresponds with the states (VA for Virginia). If the state doesnt exist already in the data, it'll just put NA there.**
...
Yeah, it's just combining all of the columns that have the same state in the column the code made and organizing the amounts in that row descending downwards.
...
1.  Which months had the most amount of high-dollar spends but low-person-amount (only a few people donated, but they put big bucks behind him)?

2.  Which outside state sent the most money proportionally?

3.  Who are the most consistent donors? If apportioned over his entire campaign, who donated the most AVERAGE PER MONTH (or something like that)?
